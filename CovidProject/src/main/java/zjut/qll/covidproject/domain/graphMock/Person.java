...
import zjut.qll.covidproject.utils.MathUtil;
...
public class Person extends Point{
        ...
        int hospitalizedTime = 0;//住院时间
        int curedTime = new Random().nextBoolean()?new Random().nextInt(5)+15:-new Random().nextInt(5)+15;//所需治愈时间
        ...
        private void action() {
            ...
            //横向运动边界
            if (this.getX() > Constants.CITY_WIDTH || this.getX() < 0) {
                moveTarget = null;
                if (udX > 0) {
                    udX = -udX;
                }
            ...
            //纵向运动边界
            if (this.getY() > Constants.CITY_HEIGHT || this.getY() < 0) {
                moveTarget = null;
                if (udY > 0) {
                    udY = -udY;
                }
        ...
        public void update() {
            ...
            //治愈出院市民的处理
            if (state == State.FREEZE && Panel.worldTime >= hospitalizedTime + curedTime && hospitalizedTime > 0){
                Hospital.getInstance().returnBed(useBed);
                // 放回城市
                this.state = State.NORMAL;
                this.setX(city.getCenterX());
                this.setY(city.getCenterY());
                this.targetXU = MathUtil.stdGaussian(100, this.getX());
                this.targetYU = MathUtil.stdGaussian(100, this.getY());
                this.infectedTime = 0;//感染时刻
                this.confirmedTime = 0;//确诊时刻
                this.dieMoment = 0;//死亡时刻，为0代表未确定，-1代表不会病死
                this.hospitalizedTime = 0;//住院时间
                this.curedTime = new Random().nextBoolean()?new Random().nextInt(5)+15:-new Random().nextInt(5)+15;//所需治愈时间

            }

            if (state == State.CONFIRMED
                    && Panel.worldTime - confirmedTime >= Constants.HOSPITAL_RECEIVE_TIME) {
            ...
            //处理病死者
            if ((state == State.CONFIRMED || state == State.FREEZE) && Panel.worldTime >= dieMoment && dieMoment > 0) {
            ...
            //处理发病的潜伏期感染者
            if (Panel.worldTime - infectedTime > stdRnShadowtime && state == State.SHADOW) {
                state = State.CONFIRMED;//潜伏者发病
                confirmedTime = Panel.worldTime;//刷新时间
            }
            //处理未隔离者的移动问题
            action();
            //处理健康人被感染的问题
